...
- `ScanBatch` 增加 `scanId` 和 `kind`，旧扫描事件无法进入当前扫描状态；
- 主进程 Worker 回调增加当前 Worker/scanId 校验，并统一处理 done/error/exit/cancel；
- 渲染层增加 `activeScanIds`、`scanStateByRoot`，后台根扫描不再覆盖当前根；
- 自动扫描等待绑定具体 `rootId + scanId`；
- 图片批次按 image id 替换/去重；
- 标签和隐藏目录异步回写增加扫描代际校验；
- 进度消息增加 150ms 时间节流，降低高速磁盘下 IPC 压力；
- 补充批次类型断言和扫描协议回归检查。

## 本轮结论

“重新扫描后 99% 卡住”的首要生命周期风险已修复。后续若仍能复现，应优先记录具体目录、扫描开始时间、最后一次进度统计和是否触发 `scan:error`，再针对文件系统异常或动态分母算法继续优化。
